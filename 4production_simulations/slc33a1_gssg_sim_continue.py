...
continued from a .chk file that was previously generated by either slc33a1_gssg_sim_rand.py
or slc33a1_gssg_sim.py
...
1. Input:
   ...
   - Reloads a previous simulation state from a checkpoint (.chk) file
...
### Intended Use:
This script is intended to continue a molecular dynamics simulation of SLC33A1 from a checkpoint
generated from a previous simulation of the same system with or without GSSG bound in a lipid bilayer 
with waters and ions under CHARMMFF where frequent state saving is crucial to avoid loss of data due to interruptions. 
...
are correctly set in `sim_params_contd.yaml` before running the script.
