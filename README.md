...
Note: `sim_env.yml` specifies the Conda environment used to run all simulations referenced in this repository. 

### Equilibration

Prior to producing simulations that are used in analyses, the simulation system produced by CHARMMGUI (`/3equilibration/openmm/step5_input.pdb`) should be energy minimized and equilibrated. Instructions for initiating energy minimization and equilibration generated by CHARMMGUI can be found in `3equilibrated/README.md`.

### Production

The equilibrated system can be further simulated using custom scripts located in `/4production`. Instructions for initiating these simulations can be found within `/4production/README.md`.
