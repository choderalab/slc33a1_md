...
2. GSSG in a partially protonated state (net charge -2) was parameterized for simulation in the CHARMM36m forcefield using the CHARMMGUI Ligand Reader and Modeler (https://charmm-gui.org/?doc=input/ligandrm). The atom naming for GSSG in the output PDB file was used to replace the atom naming for GSSG in `/2model_preparation/slc33a1map_gssg_2.pdb`, resulting in `/2model_preparation/slc33a1map_gssg_3.pdb`.
